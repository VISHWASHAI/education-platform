...
analytics.get('/overview', async (c) => {
  ...
  const rate = (row: any) => (Number(row.total) ? Math.round((Number(row.present) / Number(row.total)) * 1000) / 10 : null);
...
analytics.get('/students-by-class', async (c) => {
  ...
  return c.json(
    // deno-lint-ignore no-explicit-any
    rows.map((r: any) => ({ classId: r.class_id, className: `${r.name} - ${r.section}`, count: Number(r.count) }))
  );
});

analytics.get('/top-performers', async (c) => {
  const { rows } = await query(
  ...
  return c.json(
    ...
    rows.map((r: any) => ({
      ...
      className: r.class_name ? `${r.class_name} - ${r.class_section}` : null,
      avgScore: Number(r.avg_score),
    }))
  );
});

analytics.get('/month-summary', async (c) => {
  const { rows } = await query(
    `SELECT
